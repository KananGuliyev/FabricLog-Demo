...
export function AppSidebarNav({
  ...
  const coreItems = items.filter((item) => item.section === "core");
  const utilityItems = items.filter((item) => item.section === "utility");

  function renderItems(sectionItems: NavigationItem[]) {
  ...
  return (
    <nav className={cn("section-stack", mobile && "mt-8")}>
      <div className="space-y-1.5">
        <p className="px-3 text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-sidebar-foreground/48">
          {tShell("coreSectionLabel")}
        </p>
